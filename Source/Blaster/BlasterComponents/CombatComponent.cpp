...
void UCombatComponent::Fire()
{
	if (CanFire()) 
	{
		...
		if (EquippedWeapon)
		{
			CrosshairShootingFactor = 1.25f;
			
			switch (EquippedWeapon->FireType) // To handle lag for firing different type of weapons, so it is shown correctly locally
			{
...
bool UCombatComponent::ServerFire_Validate(const FVector_NetQuantize& TraceHitTarget, float FireDelay)
{
	if (EquippedWeapon)
	{
		bool bNearlyEqual = FMath::IsNearlyEqual(EquippedWeapon->FireDelay, FireDelay, 0.2f);
		return bNearlyEqual;
	}
	return true;
}
...
void UCombatComponent::ShowAttachedGrenade(bool bShowAttachedGrenade)
{
	...
	{
		Character->GetAttachedGrenade()->SetVisibility(bShowAttachedGrenade);
	}
}
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			if (EquippedWeapon)
			{
				...
				float RecoilTime = EquippedWeapon->RecoilTimeline.GetPlaybackPosition();
			}
			else
			{
